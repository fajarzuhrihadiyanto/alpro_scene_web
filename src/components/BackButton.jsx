...
const BackButton = () => {
    ...
    const back = () => {
        ...
        let controlsTargetOffset = [4, 0, 0], cameraPosition = [-4,1.5,0]

        if ([FOCUS_SUBJECT, FOCUS_LECTURER].includes(focusTarget)) {
            cameraPosition = [0, 1.5, 4]
            controlsTargetOffset = [0, 0, -4]
        }

        if ([FOCUS_COMMUNITY_SERVICE, FOCUS_RESEARCH].includes(focusTarget)) {
            cameraPosition = [4,1.5,0]
            controlsTargetOffset = [-4, 0, 0]
        }

        if ([FOCUS_BOOKS].includes(focusTarget)) {
            cameraPosition = [0, 1.5, -4]
            controlsTargetOffset = [0, 0, 4]
        }
